...
export default function InputForm() {
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
      ...
      <h2 className="text-3xl font-bold text-white">Predict Your Impact</h2>
      <p className="text-slate-400">Use our new Body Impact feature for a complete personalized health analysis based on your profile and real-time environmental data.</p>
      <Link to="/body-impact">
        <motion.button whileHover={{ scale: 1.05 }} className="px-8 py-3 bg-gradient-to-r from-blue-500 to-emerald-500 text-white font-semibold rounded-xl inline-flex items-center gap-2">
          Go to Body Impact <ArrowRight size={16} />
        </motion.button>
      </Link>
    </motion.div>
  );
